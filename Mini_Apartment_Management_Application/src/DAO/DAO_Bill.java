...
public class DAO_Bill implements DAOInterface<Bill>{
	...
	public void insertData(Bill t, String s, String s1) {
		try {
			...
			String query1 = "INSERT INTO hoadon (ID, MaT, MaP, MaK, SoDien, NgayXuatHD, TongTien) "
					+ "VALUES(?, ?, ?, ?, ?, ?, ?)"; // Lệnh truy vấn vào bảng hóa đơn
			String query2 = "INSERT INTO thongkehoadon (NgayXuatHD, TongTien) "
					+ "VALUES(?, ?)"; // Lệnh truy vấn vào bảng thống kê hóa đơn
			try {
				...
				preparedStatement.setDate(1, Date.valueOf(t.getInvoiceDate()));
				preparedStatement.setBigDecimal(2, new BigDecimal(t.getBillPrice()));
				preparedStatement.execute(); // Thực hiện truy vấn
	...
	public String selectTurnover(int month, int year) {
		...
		String query = "";
		if(month != -1 && year != -1) {
			query = "Select Sum(TongTien) as TongDoanhThu From thongkehoadon"
